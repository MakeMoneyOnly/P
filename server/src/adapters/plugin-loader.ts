...
/**
 * Strips the `file://` or `file:/` prefix from packageName and returns the local path.
 * Used for local development without npm install.
 */
export function normalizePackageNameWithFilePrefix(
  packageName: string,
): { isFilePrefix: boolean; localPath: string | null } {
  // file://path → path
  if (packageName.startsWith("file://")) {
    return { isFilePrefix: true, localPath: packageName.slice(7) };
  }
  // file:/path → path (single slash case)
  if (packageName.startsWith("file:/")) {
    return { isFilePrefix: true, localPath: packageName.slice(5) };
  }
  return { isFilePrefix: false, localPath: null };
}
...
function resolvePackageDir(record: Pick<AdapterPluginRecord, "localPath" | "packageName">): string {
  ...
  const { isFilePrefix, localPath } = normalizePackageNameWithFilePrefix(record.packageName);
  if (isFilePrefix && localPath) {
...
export async function loadExternalAdapterPackage(
  ...
  // Handle file:// prefix for local development
  const { isFilePrefix, localPath: fileLocalPath } = normalizePackageNameWithFilePrefix(packageName);
  const resolvedLocalPath = localPath || (isFilePrefix ? fileLocalPath : undefined);

  const packageDir = resolvedLocalPath
    ? path.resolve(resolvedLocalPath)
